  ...
  return (
    <nav className={`navbar__container ${visible ? 'visible':''}`} ref={container}>
      ...
      <div className={`tab__group ${showSidebar ? 'show':""}`}>
           <span className="icon__container close__btn" onClick={()=>setShowSidebar(!showSidebar)}><FaTimes/></span>
           {
            menu.map((list,index)=>(
              <Link
                activeClass="active"
      ...
      <div className="column nav__buttons__group">
          ...
          <button className="btn btn__primary"><Link
            activeClass="active"
            to="contact"
            spy={true}
            smooth={true}
            ...
            duration={500}
          >Contact</Link></button>
          <button className="btn btn__primary"><FaArrowUpRightFromSquare/></button>
